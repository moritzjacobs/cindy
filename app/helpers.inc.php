...
class Helpers {
	...
	static function build_file_cache($dir = '.') {
		...
		$cache_folder_pts = explode("/", Config::$cache_folder);
		$cache_name = array_pop($cache_folder_pts);
		foreach($files as $path) {
			$file = basename($path);
			if(substr($file, 0, 1) == "." || $file == $cache_name) continue;
	...
	static function human_filesize($size, $max = null, $decimal_sep = ".") {
		...
		$ret = sprintf($retstring, $size, $sys['prefix'][$i]);
